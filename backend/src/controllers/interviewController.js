...
async function createInterview(req, res) {
  try {
    const { candidateId, resumeId, jobDescriptionId, questions: initialQuestions } = req.body || {};
    ...
    // 6b. If questions were provided, create Question records in DB
    const savedQuestionDocs = [];
    if (Array.isArray(initialQuestions) && initialQuestions.length > 0) {
      for (let i = 0; i < initialQuestions.length; i++) {
        ...
        if (text) {
          const diffRaw = (q.difficulty || "medium").toLowerCase();
          const diff = ["easy", "medium", "hard"].includes(diffRaw) ? diffRaw : "medium";
          const qDoc = await Question.create({
            interviewId: interview._id,
            questionText: text,
            referenceAnswer: q.referenceAnswer || (Array.isArray(q.expectedTopics) ? q.expectedTopics.join(", ") : ""),
            questionType: q.questionType || "technical",
            targetSkill: q.skill || q.targetSkill || "",
            difficulty: diff,
            order: i + 1,
          });
          savedQuestionDocs.push(qDoc);
          interview.questions.push(qDoc._id);
        }
      }
      await interview.save();
    }
    ...
    return res.status(201).json({
      ...
      interview: {
        ...
        questions: savedQuestionDocs.length > 0 ? savedQuestionDocs.map(qd => ({
          questionId: qd._id.toString(),
          question: qd.questionText,
          skill: qd.targetSkill,
          difficulty: qd.difficulty,
          referenceAnswer: qd.referenceAnswer,
        })) : interview.questions,
